...
namespace {
...
#if CY_INSTRUMENT
  cl::opt<std::string>
  FlipConditionFile("flipping-condition-file",
            cl::desc("UNCONSTANT_CONDITION.txt as for Flipping Target"),
            cl::init(""));
  ...
  void readFile(){
      ...
      //std::ifstream inFile("UNCONSTANT_CONDITION.txt");
      errs() << FlipConditionFile << "\n";
      std::ifstream inFile(FlipConditionFile);
      errs() << "File Process: \n";
	  ...
	  int count = 0;
         for (BasicBlock::iterator II=BB->begin(); II != BB->end(); II++){
             if (IntrinsicInst *ii = dyn_cast<IntrinsicInst>(&*II)){
                 ...
                 if (CI->isInlineAsm()) //usually happens at LLVM Pass, but we add it in KLEE as well, in case some asms are not deleted by KLEE
                     count--;
                 else{
                     Function *CF = CI->getCalledFunction();
                     if (CF && (CF->getName().str() == "klee_div_zero_check" || CF->getName().str() == "klee_overshift_check" ))
                         count -= 2;
                 }
             }
...
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki) {
  ...
  if (cy_debug){
      if (isa<LoadInst>(i)){
          if (getBBOffsetStr(cast<Instruction>(i)) == "b322:14")
              i->dump();
      }
  }
#endif
  ...
  case Instruction::ICmp: {
    ...
    std::string FunctionName = i->getParent()->getParent()->getName();
    //if (FileName.find("Model0_tcp_input") == -1)
    if (FunctionName.find("Model") == -1)
        break;
    std::string Model_index = FunctionName.substr(0,6);
    int target_index;
    int target_val[2];
    int OpID;
    ...
    std::string Prefix = Model_index+","+getBBOffsetStr(cast<Instruction>(i));
    
    errs() << "Prefix: " << Prefix << "\n";
    if (Prefix.find("b1433") != -1){
        errs() << "Access b1433\n";
    ...
    std::string target_str_tmp = Modify_Target[Prefix];
    target_index = target_str_tmp[0]-'0';
    std::string target_str = target_str_tmp.substr(2,target_str_tmp.length()-2);
    ...
    */
    ...
    bool tmp_flag = false;
    tmp_pos = target_str.find("#");
    tmp_pos2 = target_str.rfind("#");
    target_val[0] = std::stoi(target_str.substr(0,tmp_pos));
    target_val[1] = std::stoi(target_str.substr(tmp_pos+1, tmp_pos2-tmp_pos-1));
    OpID = std::stoi(target_str.substr(tmp_pos2+1, target_str.length()-tmp_pos2-1));
    errs() << "target_val0: " << target_val[0] << "\n";
    errs() << "target_val1: " << target_val[1] << "\n";
    errs() << "OpID: " << OpID << "\n"; //0: ave, max; 1: min, ave; 2: min, max

    int flip_counter = 0;
        if (OpID == 0 || OpID == 1){ //ave -> x > min && x < max
            // constant expr -1 would cause error, so currently we force to add (unsigned int), assumming all target_val is unisgned int
            // TODO: check APInt function as mentioned in https://github.com/klee/klee/issues/803
            // TODO: somehow KLEE cannot resolve 0 < (ZExt w32 (ReadLSB w16 0 *((unsigned short *)((char *)&Model0_Server_A+1182)) )) < 2; it can resolve case 1 < S < 3 btw.
            ref<Expr> cy_result = SgtExpr::create(target, ConstantExpr::create((unsigned int)target_val[0], target->getWidth()));
            ref<Expr> cy_result2 = SltExpr::create(target, ConstantExpr::create((unsigned int)target_val[1], target->getWidth()));
#if CY_PATH_CONSTRAINTS
            ExecutionState tmp(state);
#else
            std::vector<ref<Expr>> tmp_exp;
            tmp_exp.push_back(cy_result);
            tmp_exp.push_back(cy_result2);
            ExecutionState tmp(tmp_exp);
#endif
            //consider all constraints alone the path
            tmp.addConstraint(cy_result);
            tmp.addConstraint(cy_result2);
            ...
            errs() << "CY_Target"+std::to_string(flip_counter);
            for (int i = 0; i < cyy_values.size(); i++){
                ...
                if (setOfNames.find(state.symbolics[i].second->name) != setOfNames.end()){
                    tmp_flag = true;
                    Prefix_Hash.insert(Prefix+"#"+std::to_string(flip_counter));
        ...
        if (OpID == 0 || OpID == 2){ //max -> x >= max
            ref<Expr> cy_result = SgeExpr::create(target, ConstantExpr::create((unsigned int)target_val[1], target->getWidth()));
#if CY_PATH_CONSTRAINTS
            //consider all constraints alone the path
            ExecutionState tmp(state);
#else
        ...
        if (OpID == 1 || OpID == 2){ //min -> x <= min
            ref<Expr> cy_result = SleExpr::create(target, ConstantExpr::create((unsigned int)target_val[0], target->getWidth()));
#if CY_PATH_CONSTRAINTS
            //consider all constraints alone the path
            ExecutionState tmp(state);
#else
...
void Executor::executeMemoryOperation(ExecutionState &state,
  ...
  if (isWrite){
      errs() << "\tby store\n";
      address->dump();
      // Seems either way it's not symbolized
  }
  else
      errs() << "\tby read\n";
#endif
