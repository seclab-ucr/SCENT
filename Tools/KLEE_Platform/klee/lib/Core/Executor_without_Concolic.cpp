...
namespace {
  ...
  std::unordered_set<std::string>Prefix_Hash;
  std::ofstream outFile2;
  std::map<unsigned long long, const Array *>ARRAY_MAP;
  std::map<unsigned long long, int>ARRAY_MAP_WIDTH;
  const Array * LAST_ARRAY;

  long long buffer2Digits(std::vector<unsigned char> buffer, int size){
      long long a = 0;
      long long base = 0;
      for (int i = 0; i < size; i++){
          a |= (unsigned char)(buffer[i]) << base;
          base += 8;
      }
      return a;
      /*
  ...
  std::string getFile(Instruction* I){
      if (MDNode *N = I->getMetadata("dbg")) { // this if is never executed
          ...
          std::string file = Loc->getFilename().data();
          unsigned line = Loc->getLine();
          return "File:"+file; //+":"+std::to_string(line);
      }
      else
          return "-";
  }
  std::string getLine(Instruction* I){
      if (MDNode *N = I->getMetadata("dbg")) { // this if is never executed
          DILocation* Loc = I->getDebugLoc();
          //std::string file = Loc->getFilename().data();
          unsigned line = Loc->getLine();
  ...
  void readFile(){
      std::string line;
      std::ifstream inFile("../CY_0_NEW_INPUT_v5.txt");
      errs() << "File Process: \n";
      ...
      std::ifstream inFile2("HASHED_NEW_INPUT_v5.txt");
      while (getline(inFile2, line)){
          Prefix_Hash.insert(line);
          errs() << "Prefix_Hash: " << line << "\n";
      }
      inFile.close();
      inFile2.close();
      outFile2.open("HASHED_NEW_INPUT_v5.txt", std::ofstream::app);
  }

  std::string getBBOffsetStr(Instruction* I){
      BasicBlock *BB = I->getParent();
      int count = 1;
      for (BasicBlock::iterator II=BB->begin(); II != BB->end(); II++){
          if (isa<CallInst>(&*II) || isa<CastInst>(&*II)){
              if ((*II).getName().str().find("CY_INST") != -1){
                count--;
              }
          }
          if (&*II == I){
      ...
      return BB->getName().str() + ":" + std::to_string(count);
  }
  
  void ExprGetSymbolNames(klee::ref<klee::Expr> e){
      if (e->getKind() == Expr::Read){
          std::string Name = (cast<ReadExpr>(e))->updates.root->name;
          setOfNames.insert(Name);
          return;
      }
      int n =  e->getNumKids();
      for (int i = 0; i < n; i++){
          ExprGetSymbolNames(e->getKid(i));
      }
  }

  bool IsDirectSymbol(klee::ref<klee::Expr> e, std::string &Name, int &result_index){
      if (e->getKind() == Expr::Read){
          Name = (cast<ReadExpr>(e))->updates.root->name;
          if (klee::ConstantExpr *new_CE = dyn_cast<klee::ConstantExpr>((cast<ReadExpr>(e))->index)) {
              std::string tmp;
              new_CE->toString(tmp);
              errs() << "ConstantExpr inside ReadExpr: " << tmp << "\n";
              result_index = std::min(result_index, std::stoi(tmp));
          }
          return true;
      }
      bool flag = false;
      if (e->getKind() == Expr::Concat || e->getKind() == Expr::ZExt || e->getKind() == Expr::SExt){
          int n = e->getNumKids();
          for (int i = 0; i < n; i++){
              if (IsDirectSymbol(e->getKid(i), Name, result_index))
                  flag = true;
          }
      }
      return flag;
  ...
  std::map<unsigned long long, const Array *>::iterator ARRAY_MAP_Search(unsigned long long addr_left, int bytes_num, int &array_index){
      unsigned long long last_addr = 0;
      if (addr_left < ARRAY_MAP.begin()->first)
          return ARRAY_MAP.end();
      for (std::map<unsigned long long, const Array *>::iterator it = ARRAY_MAP.begin(), eit = ARRAY_MAP.end(); it != eit; it++){
          if (it->first == addr_left){
              array_index = 0;
              return it;
          }
          else if (it->first < addr_left){
              if (addr_left+bytes_num <= it->first+ARRAY_MAP_WIDTH[it->first]){
                  array_index = addr_left - it->first;
                  errs() << "ARRAY_MAP_Search in the range\n";
                  return it;
              }
          }
          else
...
#if CY_DEBUG
  if (isSeeding){
      errs() << "seedMap size: " << it->second.size() << "\n";
      errs() << "true seedMap size: " << seedMap.size() << "\n";
  }
#endif
...
#if CY_DEBUG
    //errs() << "DEBUG: SeedInfo_size " << it->second.size() << "\n";
#endif
...
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki) {
  ...
  case Instruction::ICmp: {
    ...
    std::string FileName = getFile(i);
    if (FileName.find("Model0_tcp_input") == -1)
        break;
    int target_index;
    int target_val[2];
    int tmp_pos;
    //We ignore the file name Index now (since basic block ID is unique, or with function name[future])
    //std::string Prefix = FileName+","+getBBOffsetStr(cast<Instruction>(i));
    std::string Prefix = "Model0,"+getBBOffsetStr(cast<Instruction>(i));
    
    errs() << "Prefix: " << Prefix << "\n";
    if (Prefix.find("b296") != -1){
        errs() << "CY_Target|*((unsigned int *)((char *)&Model0_Server_L+444)) should be matched\n";
        //errs() << "Hash: " << Prefix_Hash.count(Prefix) << "\n";
    }
    //errs() << "Line: " << getLine(i) << "\n";
    if (Modify_Target.find(Prefix) == Modify_Target.end())
        break;
    //else
    //    errs() << "Matched\n";

    std::string target_str = Modify_Target[Prefix];
    target_index = target_str[0]-'0';

    errs() << "Matched: " << *i << "\n";
    errs() << "target_index: " << std::to_string(target_index) << "\n";
    ref<Expr> target = eval(ki, target_index, state).value;
    if (target->getKind() == Expr::Constant){
        errs() << "target is Constant: ";
        errs() << target_index << "\n";
        target->dump();
        break;
    }
    else{ //Just for debug purpose
        errs() << "target is symbolized\n";
        errs() << target_index << "\n";
        target->dump();
    }
    if (Prefix_Hash.count(Prefix) != 0)
        break;

    target->dump();
    setOfNames.clear();
    ExprGetSymbolNames(target);
    /*
    target->dump();
    for (std::set<std::string>::iterator it=setOfNames.begin(); it != setOfNames.end(); it++){
        errs() << *it << " ";
    }
    errs() << "\n";
    */
    ...
    bool tmp_flag = false;
    tmp_pos = target_str.rfind("#");
    target_val[0] = std::stoi(target_str.substr(2,tmp_pos-2));
    target_val[1] = std::stoi(target_str.substr(tmp_pos+1, target_str.length()-tmp_pos-1));
    for (int i = 0; i < 2; i++){
        errs() << "Flip to " << target_val[i] << "\n";
        ref<Expr> cy_result = EqExpr::create(target, ConstantExpr::create(target_val[i], target->getWidth()));
       
        ExecutionState tmp(state);
        tmp.addConstraint(cy_result);
        std::vector< std::vector<unsigned char> > cyy_values;
        std::vector<const Array*> cyy_objects;
        for (unsigned i = 0; i != state.symbolics.size(); ++i)
            cyy_objects.push_back(state.symbolics[i].second);
        solver->getInitialValues(tmp, cyy_objects, cyy_values);
        //errs() << "symbolics size: " << state.symbolics.size() << " | value size: " << cyy_values.size() << "\n";
        errs() << "CY_Target";
        for (int i = 0; i < cyy_values.size(); i++){
            //errs() << "CYY_Name: " << state.symbolics[i].second->name << "\n";
            if (setOfNames.find(state.symbolics[i].second->name) != setOfNames.end()){
                tmp_flag = true;
                errs() << "|" << state.symbolics[i].second->name << "#" << buffer2Digits(cyy_values[i], state.symbolics[i].second->size);
            }
        }
        errs() << "\n";
    }
    if (tmp_flag){
        Prefix_Hash.insert(Prefix);
        outFile2 << Prefix << "\n";
    }
#endif
    break;
  ...
  case Instruction::Store: {
    ...
    value->dump();
#endif
    if (i->getParent()->getParent()->getName() == "Model0_Symbolize"){
        //address (base)-> convert to string
        //array address (LAST_ARRAY)
        std::string address_str;
        cast<ConstantExpr>(base)->toString(address_str);
        //ARRAY_MAP.insert(std::pair<std::string, const Array *>(address_str, LAST_ARRAY));
        ARRAY_MAP[std::stoull(address_str)] = LAST_ARRAY;
        ARRAY_MAP_WIDTH[std::stoull(address_str)] = value->getWidth()/8;
        //CY_SYMBOLIZE_INITIALIZATION = true;
//#if CY_DEBUG
        errs() << "Inside Model0_Symbolize\n";
        errs() << "address: " << std::stoull(address_str) << "\n";
//#endif
    }
    if (i->getParent()->getName() == "b324"){
        errs() << "Inside b324\n";
        DEBUG_CY = true;
    }
...
void Executor::run(ExecutionState &initialState) {
  ...
  if (usingSeeds) {
    ...
    while (!seedMap.empty()) {
      ...
      //errs() << "function name: " << ki->inst->getParent()->getName() << ": " << ki->inst->getParent()->getParent()->getName() << "\n";
      ...
      if ((stats::instructions % 1000) == 0) {
        ...
        if (seedTime && time > startTime + seedTime) {
          ...
          klee_message("%d seeds remaining over: %d states", 
                       numSeeds, numStates);
          errs() << "name: " << ki->inst->getParent()->getParent()->getName() << "\n";
          //errs() << "File: " << getFile(ki->inst) << " : " << getLine(ki->inst) << "\n";
...
void Executor::executeMemoryOperation(ExecutionState &state,
  ...
  //TODO: target -yue
  // Follow the load way to check whether the object from the address is symbolic
  // if it is symbolic and its value is constant:
  //    update seed with this value, but not update symbol
  //    //what if it's memset? then it will be complicated
...
#if CY_DEBUG
        errs() << "is Write. ObjectState:\n";
        os->print();
        errs() << "result: ";
        result->dump();

        errs() << "result type: " << result->getKind() << "\n";

#endif
        if (result->getKind() != Expr::Constant && address->getKind() == Expr::Constant){
//#if CY_DEBUG
//#endif
            if (mo->name == "Model0_Req_A.Model0_snt_isn"){
                errs() << "Model0_Req_A.Model0_snt_isn is written\n";
                result->dump();
                value->dump();
                errs() << "-----\n";
            }
            if (ConstantExpr *value_CE = dyn_cast<ConstantExpr>(value)) {
                    /*
                    for (unsigned i = 0; i != state.symbolics.size(); ++i) {
                        errs() << "inside state.symbolics: " << state.symbolics[i].first << "\n";
                        errs() << "array address: " << state.symbolics[i].second << "\n";
                        if (mo == state.symbolics[i].first)
                            errs() << "name: " << state.symbolics[i].second->name << "\n";
                    }*/
                if (DEBUG_CY){
                    errs() << "Symbole\n";
                    os->print(); //any field will print the whole object
                    errs() << "value width: " << value->getWidth() << "\n";
                    errs() << "result width: " << value->getWidth() << "\n";
                    errs() << "result: ";
                    result->dump();
                    errs() << "Name: " << mo->name << "\n";
                    errs() << "mo address: " << (unsigned long long)(mo) << "\n";
                    errs() << "current address: " << address << "\n";
                    errs() << "offset: ";
                    offset->dump();
                }
                ...
                //find array:
                std::string address_str;
                cast<ConstantExpr>(address)->toString(address_str);
                unsigned long long address_ull = std::stoull(address_str);
                errs() << "address_ull: " << address_ull << "\n";
                int array_index;
                        
                unsigned total_numByte = (value->getWidth()+7)/8;//get total_numByte (not sure os->size is correct)
                
                std::map<unsigned long long, const Array *>::iterator it;
                it = ARRAY_MAP_Search(address_ull, total_numByte, array_index);
                if (it != ARRAY_MAP.end()){
                    const Array *array = it->second;
                    std::map< ExecutionState*, std::vector<SeedInfo> >::iterator seed_it =
                        seedMap.find(&state); //This seems okay. When state is newly forked, it will be push_backed into seedMap
                    if (seed_it != seedMap.end()){
                        //if (array_index % 8){
                            errs() << "array_index: " << array_index << "\n";
                            errs() << "value->width: " << value->getWidth() << "\n";
                            errs() << "start address: " << address_ull << "\n";
                            errs() << "end address: " << address_ull+total_numByte << "\n";
                            errs() << "symbol initial address: " << it->first << "\n";
                        //}

                        SeedInfo &si = *(seed_it->second.begin()); //double check if begin is enough
                        if (seed_it->second.size() > 1)
                            errs() << "ASSERT: Seed Infor number: " << seed_it->second.size() << "\n";

                        std::vector<unsigned char> &new_values = si.assignment.bindings[array]; //get array
                        
                        errs() << "Found:\n";
                        result->dump();
                        errs() << "value: ";
                        value->dump();
                        errs() << "array name: " << array->name << "\n";
                        DEBUG_CY = true;

                        //convert value to value_bytes
                        std::string tmp;
                        value_CE->toString(tmp);
                        unsigned long long temp_value = std::stoull(tmp);
                        unsigned char *value_bytes = (unsigned char *)malloc(total_numByte); 
                        //for (int i = 0, index=0; i < total_numByte; i++, index+=8)
                        //    new_values[array_index+i] = temp_value>>index;
                            //value_bytes[i] = temp_value>>index;
                        //new_values.insert(new_values.begin()+array_index, value_bytes, value_bytes + total_numByte);
                    }
                }
                

                //filter symbols based on added seed names. The other symbols dont have seeds
                /*
                std::string result_name;
                int result_index = 999999;
                if (IsDirectSymbol(result, result_name, result_index)){
                    if (state.arrayNames.count(result_name)){
                        std::string tmp;
                        value_CE->toString(tmp);
                        ...
                        for (int i = 0, index=0; i < total_numByte; i++, index+=8)
                            value_bytes[i] = temp_value>>index;
                        //don't write concrete value into objectstate
                        //Instead, just update seed (KTestObject)
                        std::map< ExecutionState*, std::vector<SeedInfo> >::iterator it =
                            seedMap.find(&state); //This seems okay. When state is newly forked, it will be push_backed into seedMap
                        if (it != seedMap.end()){

                            SeedInfo &si = *(it->second.begin()); //double check if begin is enough
                            if (it->second.size() > 1)
                                errs() << "ASSERT: Seed Infor number: " << it->second.size() << "\n";
                            for (Assignment::bindings_ty::const_iterator ait = si.assignment.bindings.begin(), aie = si.assignment.bindings.end(); ait != aie; ait++){
                                const Array *array = ait->first;
                                if (ait->first->name == result_name){
                                    errs() << "ait->first->name: " << ait->first->name << "\n";
                                    errs() << "Found name: " << result_name << "\n";
                                    result->dump();
                                    errs() << "value: ";
                                    value->dump();
                                    errs() << "result index: " << result_index << "\n";
                                    errs() << "total_numByte: " << total_numByte << "\n";
                                    DEBUG_CY = true;
                                    std::vector<unsigned char> &new_values = si.assignment.bindings[array]; //get array
                                    //new_values.insert(new_values.begin()+result_index, value_bytes, value_bytes + total_numByte);
                                    errs() << "~~~~~~~~\n";
                                }
                            }
                            //if (DEBUG_CY)
                            ...
                            errs() << "ASSERT: state not in seedMap\n";
                        }
                    }
                }
                */
...
#if CY_DEBUG
    //errs() << "Success: outBounds\n";
#endif
  } 
#if CY_DEBUG
      //CY:
      //errs() <<"address Space resolveOne fails\n";
#endif
...
//#if CY_DEBUG
    errs() << "makesymbolic:\n";
    errs() << name << ":" << (unsigned long long)mo << "\n";
//#endif
...
#if CY_DEBUG
              errs() << "assignment bindings 1: \n";
              si.assignment.dump();
              errs() << "assignment bindings end\n";
#endif
            if (ZeroSeedExtension) {
