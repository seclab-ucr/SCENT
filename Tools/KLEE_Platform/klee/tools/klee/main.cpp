...
namespace {
  ...
  CheckDivZero("check-div-zero",
               ...
               cl::desc("Inject checks for division-by-zero (default=false)"), //Yue
               cl::init(true),
               cl::cat(ChecksCat));
  ...
  CheckOvershift("check-overshift",
                 ...
                 cl::desc("Inject checks for overshift (default=false)"), //Yue
                 cl::init(true),
                 cl::cat(ChecksCat));
...
#if CY_DEBUG
    errs() << "seed size: " << seeds.size() << "\n";
    errs() << "SeedOutFile size: " << SeedOutFile.size() << "\n";
#endif
    for (std::vector<std::string>::iterator
