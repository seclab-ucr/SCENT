...
process_klee ()
{
    ...
    if [ ! -s klee_result_${index}_0.txt ] && [ -s klee_result_${index}_1.txt ]
    then
        # klee_no_path_build/bin/klee won't consider constraints alone the path
...
if [ "$Handshake" = "true" ] ; then
    ...
    echo ${Handshake}
    echo "3whs false"
    cp $CODE_PATH/Model0_global.c CY_Model0_global.c
    cp $CODE_PATH/Model1_global.c CY_Model1_global.c
    cp $CODE_PATH/Model0_Server_Initialize.c CY_Model0_Server_Initialize.c
    cp $CODE_PATH/Model0_Server_A_Initialize.c CY_Model0_Server_A_Initialize.c
    cp $CODE_PATH/Model1_Server_Initialize.c CY_Model1_Server_Initialize.c
    cp $CODE_PATH/Model1_Server_A_Initialize.c CY_Model1_Server_A_Initialize.c
...
do
    ...
    opt -load IsConstantLib.so -IsConstant ${CUR}.bc 1> temp.bc 2> debug_v4.txt
    ...
    if [ $Line_Count1 -ne $(( $Line_Count2/2 )) ]; then
        ...
        echo "  >>Start Flip Initialization One By One"
            ...
            do
                ...
                python ${SCENT_PATH}/Script/Update_Initialization_3whs.py $CUR $NEXT ${CODE_PATH} ${LLVM_PATH} > tmp.txt
                ...
                if cat tmp.txt | grep -q "Success"; then
                    sh ${SCENT_PATH}/Script/generate_new_bc_3whs.sh $NEXT $CUR &
    ...
    do
        ...
        if cat temp.log | grep -q "Success"; then # If it's always successful, then we can include opt inside test3.sh
            sh ${SCENT_PATH}/Script/generate_new_bc_constant_3whs.sh $NEXT $CUR &
            #P_$Right=$!
            echo "$NEXT is generated"
            if [ $PRIORITY -eq 0 ]; then
                echo ${NEXT} >> BFS_QUEUE_FIRST.txt
