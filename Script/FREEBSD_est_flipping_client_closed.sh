...
#Configurable
INDEX=FREEBSD_EST_3pkt_port_client_closed_V1
Debug=true
Debug0=true
Handshake=false
PROCESS_LIMIT=30
...
CODE_PATH=~/Work/Linux_kernel/Replace/Target_Model/LLVM_Version/FreeBSD_Source_Code/abstracted_source_code/TCP_Model_Client #folder for source codes
...
if ${Debug0}; then
    ...
    spin -a new_model_EST_debug
    cp new_model_EST_debug ${BIN_PATH}/
...
do
    ...
    if [ $Line_Count1 -ne $(( $Line_Count2/2 )) ]; then
        ...
        echo "  >>Start Flip Initialization One By One"
            ...
            do
                ...
                python ../Script/Update_Initialization_EST.py $CUR $NEXT ${CODE_PATH} > tmp.txt
                ...
                if cat tmp.txt | grep -q "Success"; then
                    sh ../Script/generate_new_bc_EST_freebsd.sh $NEXT $CUR &
    ...
    do
        ...
        if cat temp.log | grep -q "Success"; then # If it's always successful, then we can include opt inside test3.sh
            sh ../Script/generate_new_bc_constant_EST.sh $NEXT $CUR &
            #P_$Right=$!
            echo "$NEXT is generated"
            if [ $PRIORITY -eq 0 ]; then
                echo ${NEXT} >> BFS_QUEUE_FIRST.txt
