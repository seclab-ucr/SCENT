...
# --------------------------------------------------
#Version 1.0
#Add transfered program only if it is a new flipping branch condition
#Version 2.0
# Not just modify initialization, but also modify constants as well. Currently we haven't found any false positive yet.
#Version 3.0
# Use packet trace, instead of the whole model checker to do DTA. For more details: collect packet trace that would reach target operand first, then just feed such packet to model checker (use model checker original formate, or just use packet order? In our case, we should use the latter to save time)
#Version 4.0
# Because we use -DBITSTATE to compile pan, we run binary with option -w37 (6.4%), -w38 (13%), -w39 (26%), -w40 (52%)

#Configurable
INDEX=FREEBSD_3WHS_3pkt_port_V6
Debug=true
Debug0=true
Handshake=true
PROCESS_LIMIT=30
...
#CODE_PATH=../Source_Code_abstracted2/
CODE_PATH=~/Work/Linux_kernel/Replace/Target_Model/LLVM_Version/FreeBSD_Source_Code/abstracted_source_code/TCP_Model #folder for source codes
TEMP_PATH=~/Work/Linux_kernel/Replace/Target_Model/LLVM_Version/
BIN_PATH=${TEMP_PATH}${INDEX}
...
process_klee ()
{
    ...
    do
        ...
        ~/Work/KLEE_Platform/klee_build_dir2/bin/klee -only-replay-seeds -seed-out file.bout -flipping-condition-file UNCONSTANT_CONDITION_${index}.txt klee_test.bc $line 1> cy_tmp.txt  2>${index}_klee_result.txt
    ...
    then
        ...
        ~/Work/KLEE_Platform/klee_build_dir3/bin/klee -only-replay-seeds -seed-out file.bout -flipping-condition-file UNCONSTANT_CONDITION_${index}.txt klee_test.bc $argument 1> cy_tmp.txt  2>${index}_klee_result.txt
...
mkdir $BIN_PATH
cd $BIN_PATH
cp ${CODE_PATH}/Makefile_build_3WHS ./Makefile
mkdir ERROR_DEBUG
rm Edge_set.log
touch Edge_set.log
rm Branches_set.log
...
if ${Debug0}; then
    cd ${CODE_PATH}
    #spin -a new_model_3WHS_new
    #cp new_model_3WHS_new ${BIN_PATH}/
    spin -a new_model_3WHS_debug
    cp new_model_3WHS_debug ${BIN_PATH}/
...
if [ "$Handshake" = "true" ] ; then
    echo "3whs true"
    cp $CODE_PATH/Model0_Globals_Initialize.c CY_Model0_global.c
    cp $CODE_PATH/Model1_Globals_Initialize.c CY_Model1_global.c
    cp $CODE_PATH/Model0_Listen_INP_INIT_v2.c CY_Model0_Listen_Server_Initialize.c
    cp $CODE_PATH/Model1_Listen_INP_INIT_v2.c CY_Model1_Listen_Server_Initialize.c
    ...
    cp $CODE_PATH/Model0_Listen_INP_INIT_v2.c CY_Model0_Listen_Server_Initialize.c
    cp $CODE_PATH/Model1_Listen_INP_INIT_v2.c CY_Model1_Listen_Server_Initialize.c
    cp $CODE_PATH/Model0_INP_INIT_v2.c CY_Model0_Server_Initialize.c
    cp $CODE_PATH/Model0_A_INP_INIT_v2.c CY_Model0_Server_A_Initialize.c
    cp $CODE_PATH/Model1_INP_INIT_v2.c CY_Model1_Server_Initialize.c
    cp $CODE_PATH/Model1_A_INP_INIT_v2.c CY_Model1_Server_A_Initialize.c
...
do
    ...
    #python ../Script/input_generation.py new_input0_v4.txt \&> NEW_INPUT_v4.txt #Actually I didn't use input1.txt, since its information is not accurate. Instead, I print all appeared values with constant value
    ../Script/TEXTFILE_PROCESS.sh $PROCESS_LIMIT
    ...
    python ../Script/Calculate_cov.py meta_info.txt \&> coverage.txt
    mv Edge_set_bak.log Edge_set.log

    Cur=$(cat coverage.txt | grep "Coverage: ")
    echo "  >>Current Culmutative Coverage: $Cur"

    python ../Script/NEW_INPUT_Parse_Min_Max.py NEW_INPUT_v4.txt > NEW_INPUT_v2.txt
    ...
    python ../Script/Calculate_BBs.py NEW_INPUT_v2.txt  Branches_BB_set.log Branches_BB_set_bak.log $Model1_offset $Model0_end > NEW_INPUT_v5.txt
    ...
    if [ $Line_Count1 -ne $(( $Line_Count2/2 )) ]; then
        ...
        BRANCH_LIMIT=$(python ../Script/KLEE_Target_Filter_new.py NEW_INPUT_v5.txt NEW_INPUT_v6.txt PACKET_TRACK.txt 2>&1) #TARGET_TRACKS.txt
        ...
        echo "  >>Start Flip Initialization One By One"
            ...
            do
                ...
                echo $PRIORITY
                SYM_ID=$(python ../Script/Symbolic_ID_lookup.py FLIP.txt Symbolic_ID.log 2>&1)
                echo SYM$SYM_ID
                NEXT=${CUR}_SYM${SYM_ID}
                python ../Script/Update_Initialization_3whs.py $CUR $NEXT ${CODE_PATH} > tmp.txt
                ...
                if cat tmp.txt | grep -q "Success"; then
                    sh ../Script/generate_new_bc_3whs_freebsd.sh $NEXT $CUR &
    ...
    do
        ...
        if cat temp.log | grep -q "Success"; then # If it's always successful, then we can include opt inside test3.sh
            sh ../Script/generate_new_bc_constant_3whs.sh $NEXT $CUR &
            #P_$Right=$!
            echo "$NEXT is generated"
            if [ $PRIORITY -eq 0 ]; then
                echo ${NEXT} >> BFS_QUEUE_FIRST.txt
